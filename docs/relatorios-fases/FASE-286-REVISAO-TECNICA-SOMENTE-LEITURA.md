...
- A migration 287 instala o bloqueio de `INSERT`, `UPDATE` e `DELETE` em tabelas públicas para sessões de revisão, inclusive se alguma política legada permitir escrita ao papel `authenticated`. A conta usa `usuarios.perfil = parceiro`, que não ativa o helper legado `is_staff()`.
...
- Migrations 286 e 287 aplicadas em produção.
