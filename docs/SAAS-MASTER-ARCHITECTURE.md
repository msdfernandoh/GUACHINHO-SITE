...
O projeto do tour guiado estabelece uma demonstração comercial de 11 paradas, do evento com check-in e sorteio promocional simulado ao lead, grupo, proposta, venda, comissão e caixa, incluindo a captação pelo “Seja Parceiro” e a gestão do modelo solicitado versus perfil de comissão vigente. A master Sorriso serve ao piloto interno; interessados externos usarão uma credencial demo compartilhada, restrita à leitura de dados fictícios em tenant separado, com progresso independente por sessão. A biblioteca “Explorar por área” explica benefício, controle e modo de uso dos módulos disponíveis. Depois do contrato, o dono recebe a trilha executiva na própria master e cada usuário recebe lições segundo seu vínculo `empresa_usuarios`, papel, plano e permissões efetivas; o progresso real é isolado por usuário e `empresa_id`. O guia não concede permissões nem altera regras operacionais. A Plataforma tem escopo global próprio e só é explicada, não navegada, pelo login demo. O planejamento, as fases e os critérios de aceite estão em `docs/projetos/TOUR-GUIADO-SAAS-ERP.md`; registro desta fase em `docs/relatorios-fases/PROJETO-2026-09-26-TOUR-GUIADO-SAAS-ERP.md`.
...
1. **Reconciliação de Vendas Fechadas e Auditoria Interativa:** Reconciliação bidirecional entre o ERP (`public.vendas`) e o CRM (`public.leads`), integrando o faturamento real do mês corrente (inclusive a venda de R$ 2.800.000,00 de Daiana Caruline Tasso) aos indicadores do funil e abrindo o modal interativo `CrmVendasFechadasModal` com detalhamento cota a cota (Migration 240).
2. **Separação Estrita de Crédito vs. Parcela:** Eliminação de contaminação de parcelas no valor de crédito dos negócios. A coluna `valor_parcela numeric(15, 2)` foi criada em `public.leads` (Migration 241) com saneamento de leads legados, permitindo que cada oportunidade armazene e exiba de forma isolada tanto o Crédito Pretendido (R$) quanto a Parcela Mensal (R$/mês) no Kanban, no Funil 3D, nos simuladores, no cadastro rápido e na edição de leads.
3. **Design Corporativo e Deconflito de Cores Racon Sinop:** Refatoração completa do design system para o modelo `raconsinop.com.br` (`tenant-admin-racon`). Cards da fila operacional e alertas agora utilizam tons pastéis de alto contraste com tipografia escura (#0f172a / #334155), inputs e formulários operam em fundo branco e o bloco do Funil 3D Executivo permanece preservado em degradê Deep Navy Racon (`#0c2340` a `#071526`) com iluminação neon verde e ciano.

### Evolução operacional 257 — Deduplicação estrita de leads por telefone, consolidação de histórico e fase Novo lead

A plataforma passa a operar com deduplicação atômica estrita de leads por telefone normalizado (Migrations 238 e 239). Quando um lead reingressa no funil por qualquer canal (formulários do site, calculadoras, eventos, sorteios ou IA), se ele estiver em andamento (não ganho), suas informações são unificadas sem duplicação: a etapa é obrigatoriamente reposicionada para *Novo lead* (`novo_lead`), os campos comerciais são atualizados com os valores recentes, todas as abordagens anteriores são preservadas cronologicamente em `historico_cadastros` e o campo `observacoes` retém exclusivamente a última observação informada. A única exceção que autoriza nova linha de lead é quando o registro anterior já se encontra no estado Ganho (*Venda fechada*), caso em que o negócio ganho permanece congelado e uma nova negociação independente é gerada para o cliente recorrente. A base de produção foi auditada e os 8 grupos de telefones duplicados legados foram consolidados com preservação de propostas e vínculos filhas.
