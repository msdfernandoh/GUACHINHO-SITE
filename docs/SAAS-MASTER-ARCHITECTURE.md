...
### Evolução operacional 258 — Reconciliação de vendas fechadas do mês e modal interativo de auditoria no CRM

O painel de indicadores e funil de vendas do CRM (`/admin/crm`) passa a operar com reconciliação canônica bidirecional entre o ERP (`public.vendas`) e o pipeline (`public.leads`), garantindo 100% de precisão nos valores de fechamento mensal. O indicador de vendas no mês e o cálculo de meta mensal consolidam todas as vendas confirmadas da empresa no mês corrente (inclusive contratações online e propostas diretas que ingressaram sem lead prévio), com data em UTC-safe. O contrato de Daiana Caruline Tasso de R$ 2.800.000,00 (18/09/2026, 14 cotas) e o de Gabriel Pereira da Costa de R$ 41.784,72 foram integrados aos leads correspondentes na etapa canônica *Venda fechada* (Migration 240) e protegidos por trigger automática `trg_vendas_sync_lead_fechamento`. O card "Vendas no mês" e as seções de conversão do funil 3D tornaram-se totalmente interativos: ao clicar, abre-se o modal `CrmVendasFechadasModal` com auditoria completa de cada negócio (cliente, crédito, parcela, prazo, data, consultor responsável, busca em tempo real e atalhos para o contrato no ERP e no Pipeline).
