...
export function InstalarAppParceiroButton({ dark = false, racon = false }: { dark?: boolean; racon?: boolean }) {
  ...
  return <div className="relative inline-flex flex-col items-center gap-3">
    <button type="button" onClick={instalar} className={`inline-flex items-center gap-2 rounded-2xl px-6 py-4 font-black shadow-lg transition hover:scale-[1.02] ${racon ? "bg-white text-blue-700" : dark ? "bg-amber-500 text-zinc-950" : "bg-zinc-950 text-white"}`}><Download className="h-5 w-5" />Baixar app de indicação</button>
