              ...
              <table className="w-full text-left text-xs">
                <thead className="border-b border-slate-200 bg-slate-50 text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-800 dark:text-slate-400">
                  <tr>
                    ...
                    <th className="p-3.5">Programa / Administradora / Tipo</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {filteredRegrasPerfis.length === 0 ? (
                    ...
                    filteredRegrasPerfis.map((regra) => {
                      const isInativa = !regra.ativa || regra.status === "INATIVA";
                      const isHomologada = !isInativa && (regra.status === "HOMOLOGADA" || regra.configuracao_homologada) && regra.status !== "RASCUNHO";
                      const isRascunho = !isInativa && (regra.status === "RASCUNHO" || (!regra.configuracao_homologada && regra.status !== "HOMOLOGADA"));

                      return (
                        <tr key={regra.id} className={`hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition ${isInativa ? "opacity-60" : ""}`}>
                          {/* Perfil */}
                          <td className="p-3.5">
                          ...
                          <td className="p-3.5">
                            <div className="font-bold text-blue-800 dark:text-blue-300">
                              {regra.programa_nome || "Programa não identificado"}
                            </div>
                            <div className="font-semibold text-slate-800 dark:text-slate-200">
                              Administradora: {regra.administradora_nome || "Racon Consórcios"}
                            </div>
                          ...
                          <td className="p-3.5 text-center">
                            ...
                            >
                              {isInativa ? "INATIVA" : isHomologada ? "HOMOLOGADA" : isRascunho ? "RASCUNHO" : regra.status}
                            </span>
                          </td>
                          ...
                          <td className="p-3.5 text-right">
                            <div className="flex items-center justify-end gap-1.5">
                              ...
                              {isInativa && canWrite && (
                                <button
                                  onClick={() => handleToggleRule(regra.id, true)}
                                  disabled={isProcessingRuleAction}
                                  className="rounded-lg border border-emerald-300 px-2.5 py-1 text-xs font-bold text-emerald-700 hover:bg-emerald-50 dark:border-emerald-800 dark:text-emerald-300 disabled:opacity-50"
                                >
                                  Reativar
                                </button>
                              )}
