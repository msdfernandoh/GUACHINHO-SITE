...
export async function ErpRepasseFranquiaPage() {
  ...
  const [grants, contas, recebimentos, previsoes, solicitacoesRes, importacoesRes, participantesRes, perfisParticipantesRes, regrasRes, gruposRes, resolucoesRes] = await Promise.all([
    ...
    db
      ...
      .select(
        "id,administradora_id,competencia,ordem_etapa,nome_etapa,valor_previsto,valor_liquidado,status,administradora:administradoras(nome),cota:cotas_definitivas(id,numero_grupo,numero_cota,ordem_cota),venda:vendas(cliente_nome,cliente:clientes(nome))",
      )
      .eq("empresa_id", empresaId)
      ...
      <RepassePdfConciliacao
        ...
        previsoes={(previsoes.data ?? []).map((row: any) => {
          ...
          return {
            ...
            ordem_cota: cota?.ordem_cota ?? null,
