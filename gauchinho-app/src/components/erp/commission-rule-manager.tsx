...
export function CommissionRuleManager({
  ...
  return (
    <section className="space-y-5 rounded-xl border border-blue-200 bg-blue-50/40 p-5">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <label className={labelClass}>
              ...
              <input
                ...
                inputMode="decimal"
                required
                placeholder={base === "credito" ? "Ex.: 2,75" : "Ex.: 1500,00"}
              />
            </label>
            <label className={labelClass}>
              Modalidade (opcional)
      ...
      <form
        action={createParticipantRuleAction}
      ...
      >
        ...
        <input
          ...
          value={JSON.stringify(
            participantStages.map((stage, index) => ({
              ...
              percentual_etapa: Number(stage.percentual.replace(",", ".")),
            })),
          )}
        />
        <div className="grid gap-3 md:grid-cols-3">
          <label className={labelClass}>
            Programa
            <select className={inputClass} name="programa_id" required>
              {programas.map((item) => (
                <option key={item.id} value={item.id}>
                  {item.nome}
                </option>
          ...
          <label className={labelClass}>
            Modo
            <select className={inputClass} name="modo_regra">
        ...
        <button className="rounded-lg bg-violet-700 px-4 py-2 text-sm font-semibold text-white">
