...
export type RepassePdfItem = {
  ...
  valor_base: number;
  valor_participante_referencia: number | null;
  venda_excluida_id: string | null;
  ...
  return <details className="rounded-lg border border-violet-200 bg-violet-50 p-2">
    ...
    <form action={action} className="mt-2 grid grid-cols-2 gap-2">
      ...
      <div className="col-span-2 grid gap-2 rounded border border-emerald-300 bg-emerald-50 p-2 sm:grid-cols-2">
        ...
        <label className="font-bold text-emerald-950">Comissão correta do vendedor<input name="valor_participante" type="number" min="0" max={Number(item.valor_comissao)} step="0.01" required defaultValue={Number(item.valor_participante_referencia ?? item.valor_comissao).toFixed(2)} className="mt-1 w-full rounded border bg-white p-1.5 font-mono"/></label>
        <p className="sm:col-span-2 text-emerald-900">Os dois valores ficam registrados separadamente e auditados, sem regra de comissão automática. A comissão do vendedor não pode ultrapassar o recebido no relatório.</p>
      </div>
