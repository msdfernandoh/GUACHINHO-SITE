...
export async function ErpAssembleiasPage({
  ...
  // Reúne todas as assembleias que pertencem ao mesmo evento sorteado
  ...
  const gruposApurados = selecionada
    ? agruparCotasPorGrupo(gruposDoEvento, cotasMapeadas, selecionada.pedra_sorteada, assembleiaIdPorGrupo)
    : [];
  ...
  return (
    <div className="space-y-6">
      ...
      {canWrite === true && (
        <form action={createAssembleiaAction} className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-5">
          <select name="grupo_id" required className="rounded-lg border bg-white px-3 py-2 font-medium">
          ...
          <input name="data_assembleia" type="date" required className="rounded-lg border px-3 py-2" />
          <input name="numero_assembleia" type="number" min="1" placeholder="Nº assembleia" className="rounded-lg border px-3 py-2" />
          <input name="pedra_sorteada" type="number" min="0" required placeholder="Pedra sorteada" className="rounded-lg border px-3 py-2" />
          <button className="rounded-lg bg-blue-700 px-4 py-2 font-semibold text-white hover:bg-blue-800">
            Registrar assembleia
          </button>
          <textarea name="observacao" placeholder="Observação operacional (opcional)" className="rounded-lg border px-3 py-2 md:col-span-5" />
        </form>
      )}
              ...
              <div className="rounded-xl border bg-white p-5">
                ...
                {gruposApurados.length > 1 && (
                  <div className="mt-4 flex flex-wrap items-center gap-2">
                    ...
                    >
                      Todos os grupos ({gruposApurados.length})
                    </Link>
                              ...
                              g.cotas.map((c) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="px-4 py-3">
                                      {isPedraExata ? (
                                        <span className="font-black text-emerald-700">0 (Exata)</span>
                                      ) : (
                                        <span className="font-bold text-slate-700">{c.distancia}</span>
                                      )}
                                    </td>
                                    <td className="px-4 py-3">
