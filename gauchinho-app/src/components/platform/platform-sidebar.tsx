...
const sections = [
  ...
  ["Franquias / Empresas", [["/platform/empresas", "Master Franquias", Building2], ["/platform/revisao", "Revisão técnica", FileStack], ["/platform/acessos-cadastro", "Acesso de revisão", Users], ["/platform/usuarios", "Usuários / Responsáveis", Users], ["/platform/dominios", "Domínios", Globe2]]],
...
export function PlatformSidebar({ restricted = false }: { restricted?: boolean }) {
  const pathname = usePathname();
  if (restricted) return <aside className="w-72 shrink-0 border-r border-slate-200 bg-slate-950 p-5 text-slate-200"><p className="text-sm font-bold text-cyan-400">Plataforma SaaS</p><nav className="mt-6"><Link href="/platform/revisao" className="block rounded-lg bg-cyan-500 px-3 py-2 text-sm font-semibold text-slate-950">Revisão técnica</Link></nav></aside>;
