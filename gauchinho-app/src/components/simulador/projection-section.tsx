  ...
  return (
    <section className={sectionCardClass()}>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-xl bg-slate-950/60 p-4">
          <p className="text-xs uppercase text-slate-500">Crédito est. em 1 ano</p>
          <p className="mt-1 text-xl font-bold">{formatCurrency(resumoAno1.creditoEstimadoReajustado)}</p>
        </div>
        <div className="rounded-xl bg-slate-950/60 p-4">
          <p className="text-xs uppercase text-slate-500">Ganho patrimonial est.</p>
        ...
        <div className="mt-4 space-y-2 sm:hidden">
          {projecao.map((l) => (
            <div key={l.ano} className="rounded-lg border border-slate-700/60 p-3 text-sm">
              ...
              <p className="text-slate-400">Crédito: {formatCurrency(l.creditoEstimadoReajustado)}</p>
              <p className="text-slate-400">Parcela: {formatCurrency(l.parcelaEstimadaNoPeriodo)}</p>
            </div>
          ))}
        </div>
      ) : null}
