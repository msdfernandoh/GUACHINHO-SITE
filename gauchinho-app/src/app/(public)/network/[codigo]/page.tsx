...
import { NetworkConviteForm } from "@/components/app-indicador/network-convite-form";
import { createAdminClient } from "@/lib/supabase/admin";
import { getResolvedTenant } from "@/lib/tenant/get-resolved-empresa";
import { isRaconModel } from "@/lib/tenant/model-family";

export default async function NetworkConvitePage({ params }: { params: Promise<{ codigo: string }> }) {
  ...
  const { data: indicador } = await db.from("programa_indicadores")
    .select("id,participante:participantes_comerciais(nome)")
    .eq("empresa_id", tenant.empresaId).eq("codigo_indicacao_curto", codigoCurto).eq("ativo", true).maybeSingle();
  ...
  return <NetworkConviteForm codigoIndicacao={codigoCurto} nomeIndicador={participante?.nome ?? null} racon={isRaconModel(tenant.siteModel)} />;
}
