...
export default function DefinirSenhaPage() {
  ...
  const [mensagem, setMensagem] = useState("Validando convite seguro...");
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(({ data }) => {
      ...
      setPronto(valido);
      setMensagem(valido ? "Crie sua senha para concluir o acesso." : "Convite inválido ou expirado. Solicite um novo convite.");
    });
  }, []);
  ...
  async function salvar(event: FormEvent<HTMLFormElement>) {
    ...
    if (ativacaoError) {
      setMensagem(`Senha definida, mas a ativação do vínculo falhou: ${ativacaoError.message}`);
      setSalvando(false);
      return;
    }
  ...
  return (
    <main className="flex min-h-[calc(100vh-8rem)] items-center justify-center px-4 py-10">
      <section className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-8 shadow-lg">
        <p className="text-xs font-bold uppercase tracking-widest text-cyan-700">Acesso à franquia</p>
        <h1 className="mt-2 text-2xl font-extrabold text-zinc-900">Definir senha</h1>
        ...
        {pronto ? (
          <form onSubmit={salvar} className="mt-6 space-y-4">
            ...
            >
              {salvando ? "Ativando acesso..." : "Definir senha e entrar"}
            </button>
          </form>
        ) : null}
      </section>
