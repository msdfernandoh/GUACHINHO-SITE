...
export default async function ImportarClientesLegadoPage() {
  ...
  const { data: programas } = racon ? await db.from("comissao_programas").select("id,nome,versao,status").eq("empresa_id",empresaAtiva.id).eq("administradora_id",racon.id).order("versao",{ascending:false}) : { data: [] };
  ...
  const programaMap = new Map((programas ?? []).map((item) => [item.id,item]));
