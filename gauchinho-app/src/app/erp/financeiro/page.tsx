...
export default async function FinanceiroCaixaPage({ searchParams }: { searchParams?: Promise<{ mes?: string; conta?: string }> }) {
  ...
  return <main className="space-y-6 p-6">
    <header className="rounded-3xl bg-slate-950 p-7 text-white shadow-xl">
      ...
      <div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-black">Financeiro &amp; Caixa</h1><p className="mt-1 text-sm text-slate-300">Saldos bancários, entradas, comissões e equalização dos sócios.</p></div><div className="flex gap-2"><Link href="/erp/conta-corrente-socios" className="rounded-xl bg-amber-400 px-4 py-2 text-xs font-black text-slate-950 shadow hover:bg-amber-300">Conta-corrente Sócios</Link><Link href="/erp/contas-pagar" className="rounded-xl bg-white/10 px-4 py-2 text-xs font-bold">Contas a pagar</Link><Link href="/erp/minhas-comissoes" className="rounded-xl bg-white/10 px-4 py-2 text-xs font-bold">Comissões</Link></div></div>
    </header>
    ...
    <section className="rounded-2xl border border-indigo-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-black">Contas internas dos sócios</h2><p className="text-xs text-slate-500">Comissões, despesas pagas pessoalmente e transferências de equalização.</p></div><Link href="/erp/conta-corrente-socios" className="text-xs font-bold text-indigo-700 hover:underline">Abrir Conta-Corrente dos Sócios (Visão Resumida / Detalhada) →</Link></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{socios.map((socio) => <div key={socio.socio_id} className="rounded-xl bg-indigo-50 p-4"><p className="text-xs font-black uppercase text-indigo-800">{socio.nome} · {Number(socio.percentual_participacao).toFixed(2)}%</p><p className="mt-2 text-2xl font-black text-indigo-950">{brl(Number(socio.saldo_interno))}</p><p className="text-[10px] text-indigo-700">Saldo interno, não saldo bancário pessoal</p></div>)}</div></section>
