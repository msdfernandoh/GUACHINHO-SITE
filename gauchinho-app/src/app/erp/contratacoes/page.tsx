  ...
  return <div className="space-y-6">
    ...
    <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <tbody className="divide-y divide-slate-100">{rows.map((r)=><tr key={r.id} className="align-top hover:bg-slate-50">
        ...
        <td className="px-4 py-4"><div className="flex flex-wrap items-center gap-2">{r.status === "FORMALIZADA" ? <>{r.clienteId && <Link className="rounded-md border px-2 py-1" href={`/erp/clientes/${r.clienteId}`}>Ver cliente</Link>}<Link className="rounded-md border px-2 py-1" href={`/erp/vendas?venda=${r.vendaId}`}>Ver venda</Link><Link className="rounded-md border px-2 py-1" href={`/erp/vendas?cota=${r.cotaId}`}>Ver cota</Link></> : <><Link className="rounded-md bg-blue-700 px-3 py-2 font-semibold text-white hover:bg-blue-800" href={`/erp/contratacoes/${r.id}`}>{podeFormalizar ? (r.status === "PENDENCIA" ? "Resolver pendência" : r.contratoAssinado ? "Conferir e formalizar" : "Ver contrato") : "Conferir contratação"}</Link><MarcarContratoAssinadoButton contratacaoId={r.id} contratoAssinado={r.contratoAssinado} contratoAssinadoEm={r.contratoAssinadoEm} formalizada={false} canAlterar={podeFormalizar || permissoes.has("gerenciar_propostas")} variant="table" /></>}</div></td>
      </tr>)}</tbody></table>
