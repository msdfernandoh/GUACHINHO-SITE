...
import { fetchAdminDepoimentos } from "../actions";
...
export default async function AdminDepoimentosPage() {
  ...
  const list = await fetchAdminDepoimentos();
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Link href="/admin/conteudo/depoimentos/novo">
          <Button>Novo depoimento</Button>
        </Link>
      </div>
      <table className="min-w-full overflow-hidden rounded-xl border text-sm dark:border-zinc-800">
          ...
          {list.map((r) => (
            <tr key={r.id} className="border-t dark:border-zinc-800">
              ...
              <td className="px-3 py-2">
                <Link href={`/admin/conteudo/depoimentos/${r.id}`} className="text-amber-600 hover:underline">
                  Editar
                </Link>
              </td>
            </tr>
