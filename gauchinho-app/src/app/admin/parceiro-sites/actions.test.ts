...
vi.mock("@/lib/parceiros/authorization", () => ({
  assertNaoPodeEditarSiteComoParceiro: (papel: string) => assertNaoPodeEditarSiteComoParceiro(papel),
...
describe("parceiro-sites actions — autorização", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("consultor sem permissão bloqueado no guard de papel", async () => {
    const { assertParceiroComercialBlockedAction } = await import("./actions");
