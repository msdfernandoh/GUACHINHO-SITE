...
export default async function PlatformGrupoPage({
  ...
  const [grupoRes, administradorasRes, historicoRes, reajustesRes] = await Promise.all([
    db
      ...
      .select(
        "id,codigo_grupo,administradora_id,tipo_administradora_id,modalidade,status,ativo,prazo_total,data_primeira_assembleia,parcelas_realizadas,prazo_restante,credito_reajustado_ate_meses,taxa_administrativa_percentual,fundo_reserva_percentual,tipo_reajuste_anual,reajuste_anual_percentual,reajuste_anual_indice,seguro_percentual,seguro_habilitado,capacidade_total,vagas_disponiveis,vagas_atualizado_em,dados_estatisticos,dados_estatisticos_atualizado_em,permite_lance_embutido,percentual_lance_embutido,percentual_parcela_reduzida,percentuais_parcela_reduzida,regra_integralizacao_parcela_reduzida,assembleia_limite_parcela_reduzida,origem_governanca,status_governanca,observacoes,updated_at,administradora:administradoras(id,nome),tipo:administradora_tipos(id,nome,codigo),modalidades:grupos_modalidades_disponiveis(id,administradora_modalidade_id,ativo,ordem,configuracao,modalidade:administradora_modalidades_comissao(id,nome,codigo,modo_reduzido_padrao,percentual_padrao,percentual_minimo,percentual_maximo)),produtos:grupos_cotas(id,valor_credito,status,ativo),categorias:grupos_categorias(categoria:catalogo_grupo_categorias(codigo,nome,ativo)),lances:grupos_modalidades_lance(id,nome,percentual_lance_embutido,percentual_recurso_proprio_minimo,base_referencia,descricao,ativo,ordem)",
      )
      .eq("id", id)
      .maybeSingle(),
