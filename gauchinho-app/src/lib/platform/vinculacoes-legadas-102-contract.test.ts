...
const migration126 = fs.readFileSync(
  path.resolve(process.cwd(), "../supabase/migrations/126_hardening_multitenant_escala_franquias.sql"),
  "utf8"
);
...
describe("Fase 102 — Vinculações Legadas dos Grupos com o Catálogo Canônico SaaS", () => {
  ...
  it("reconcilia execução parcial sem inventar empresa para histórico existente", () => {
    expect(migration126).toContain("ADD COLUMN IF NOT EXISTS empresa_id uuid REFERENCES public.empresas");
    expect(migration126).toContain("WHERE empresa_id IS NULL");
    expect(migration126).toContain("classifique os registros antes de aplicar a migration 126");
    expect(migration126).toContain("WHERE empresa_id = p_empresa_id");
    expect(migration126).toContain("ALTER COLUMN empresa_id SET NOT NULL");
  });
