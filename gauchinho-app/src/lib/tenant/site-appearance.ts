...
export const VISUAL_COLORS = {
  ...
  cor_titulo: "Títulos",
  cor_texto: "Textos",
  cor_destaque: "Destaques",
  cor_botao: "Botões / seleção",
  cor_texto_botao: "Texto dos botões",
} as const;
...
export type SitePagesAppearance = Record<
  string,
  Record<string, VisualBlockConfig>
>;
export type SiteVisualIdentity = {
...
export const HOME_BLOCKS = {
  ...
  simulador_home: "Simulador da home",
  produtos: "Seção de produtos",
  card_veiculos: "Card de veículos",
  card_imoveis: "Card de imóveis",
  card_patrimonio: "Card de patrimônio",
  educacao: "Como funciona / benefícios",
  filiais: "Unidades / filiais",
  estatisticas: "Estatísticas / embaixador",
  franquia: "Seja um franqueado",
};
export const SIMULATOR_BLOCKS = {
  pagina: "Página inteira",
  cabecalho: "Cabeçalho",
  solucao: "Escolha da solução",
  bem: "Escolha do bem",
  credito: "Valor do crédito",
  prazo: "Prazo",
  parcela: "Modalidade da parcela",
  estrategia: "Estratégia avançada",
...
export function visualBlocksForPage(page: string): Record<string, string> {
  ...
  if (page === "/grupos")
    return {
      pagina: "Página inteira",
      cabecalho: "Cabeçalho",
      tabela: "Tabela de grupos",
      totais: "Resumo / totais",
    };
  return {
    pagina: "Página inteira",
    cabecalho: "Cabeçalho / banner",
    conteudo: "Conteúdo",
  };
}
...
const variables: Record<VisualColor, string> = {
  cor_fundo: "--visual-bg",
  cor_titulo: "--visual-title",
  cor_texto: "--visual-text",
  cor_destaque: "--visual-accent",
  cor_botao: "--visual-button",
  cor_texto_botao: "--visual-button-text",
};
export function pageAppearanceCss(
  identity: SiteVisualIdentity,
  page: string,
): string {
  ...
  return Object.entries(config)
    .sort(([a], [b]) => (a === "pagina" ? -1 : b === "pagina" ? 1 : 0))
    .map(([id, block]) => {
      const selector =
        id === "pagina" ? root : `${root} [data-site-block="${id}"]`;
      const styles = (Object.keys(variables) as VisualColor[]).flatMap((key) =>
        block[key] ? [`${variables[key]}:${block[key]}!important`] : [],
      );
      if (block.cor_fundo)
        styles.push(
          `background-color:${block.cor_fundo}!important`,
      ...
      if (!photoSlot && (page === "/" || id !== "pagina") && block.imagem_url)
        styles.push(
          `background-image:url("${block.imagem_url}")!important`,
          `background-size:${block.imagem_ajuste || "cover"}`,
          `background-position:${(block.imagem_posicao || "center").replace("-", " ")}`,
        );
      const inherited =
        id === "pagina"
...
export function blockImage(
  ...
  return (
    normalizePageAppearance(identity.paginas_blocos)[page]?.[id]?.imagem_url ||
    fallback
  );
}
export function visibleModelMenus<
  T extends { id: string; ativo?: boolean; obrigatorio?: boolean },
>(catalog: T[], enabled: string[]) {
  const selected = new Set(enabled);
  return catalog.filter(
    (item) =>
      item.ativo !== false &&
      (item.obrigatorio === true || selected.has(item.id)),
  );
}
