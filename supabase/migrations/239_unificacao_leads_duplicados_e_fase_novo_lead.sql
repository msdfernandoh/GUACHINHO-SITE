-- Migration 239: Deduplicacao Estrita de Leads por Telefone, Historico Acumulado e Fase Novo Lead
-- Regra Canônica:
-- 1. Se o lead existir e NÃO estiver Ganho: NUNCA duplica.
--    - Reposiciona a etapa para 'Novo lead' (topo do funil)
--    - status = 'Novo', fechado = false, perdido_at = null
--    - Atualiza os campos comerciais com os últimos valores preenchidos
--    - Acumula novo bloco cronológico no historico_cadastros com data na frente
--    - Mantém no campo observacoes a ÚLTIMA observação informada
-- 2. Se o lead existir e ESTIVER Ganho (venda fechada): PODE duplicar.
--    - O lead ganho permanece 100% intacto e congelado na coluna 'Venda fechada'
--    - Gera uma NOVA NEGOCIAÇÃO (cópia) na etapa 'Novo lead'
...
begin
  ...
  -- CASO 1: Lead em andamento encontrado -> NUNCA DUPLICA. Atualiza, move para 'Novo lead' e acumula histórico
  if v_lead_rec.id is not null then
    ...
    if v_lead_ganho_rec.id is not null then
      ...
      insert into public.leads (
        ...
        p_payload->>'srd_responsavel_nome',
        p_payload->>'temperatura',
        p_payload->>'modelo_interesse',
        p_payload->>'proxima_acao',
        now(),
        now(),
        false
      )
