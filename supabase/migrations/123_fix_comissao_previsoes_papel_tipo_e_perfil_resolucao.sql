-- Migration 123: Correção definitiva de colunas papel_tipo/previsao_franquia_id e Resolução de Perfil de Comissão
BEGIN;

-- 1. Garantir colunas em comissao_previsoes_participantes e vendas
...
ALTER TABLE public.vendas ADD COLUMN IF NOT EXISTS perfil_principal_id uuid REFERENCES public.comissao_perfis(id) ON DELETE SET NULL;
ALTER TABLE public.vendas ADD COLUMN IF NOT EXISTS perfil_secundario_id uuid REFERENCES public.comissao_perfis(id) ON DELETE SET NULL;
ALTER TABLE public.vendas ADD COLUMN IF NOT EXISTS data_primeira_parcela date;
ALTER TABLE public.vendas ADD COLUMN IF NOT EXISTS data_segunda_parcela date;
ALTER TABLE public.vendas ADD COLUMN IF NOT EXISTS participante_secundario_id uuid REFERENCES public.participantes_comerciais(id) ON DELETE SET NULL;
ALTER TABLE public.vendas ADD COLUMN IF NOT EXISTS participante_secundario_fracao_percentual numeric(7,4);

-- 2. Atualizar rpc_converter_contratacao_venda para propagar o perfil_principal_id e percentual_franqueadora
...
BEGIN
  ...
  -- Valores financeiros da contratação
  v_credito := COALESCE(
  ...
  -- Inserir em Vendas com perfil_principal_id explícito
...
-- 3. Atualizar rpc_gerar_previsoes_comissao_v2 com suporte a papel_tipo e resolução exata de programa
...
BEGIN
  ...
  v_data_base_1 := COALESCE(v_venda.data_primeira_parcela, (v_venda.snapshot_venda->>'data_primeira_parcela')::date, (v_venda.snapshot_venda#>>'{dados_simulacao,data_primeira_parcela}')::date, v_venda.data_venda::date, CURRENT_DATE);
  v_data_base_2 := COALESCE(v_venda.data_segunda_parcela, (v_venda.snapshot_venda->>'data_segunda_parcela')::date, (v_venda.snapshot_venda#>>'{dados_simulacao,data_segunda_parcela}')::date, (v_data_base_1 + INTERVAL '1 month')::date);
  ...
  v_secundario_id := COALESCE(v_venda.participante_secundario_id, (v_venda.snapshot_venda->>'participante_secundario_id')::uuid, (v_venda.snapshot_venda#>>'{dados_simulacao,participante_secundario_id}')::uuid);
  v_fracao_secundario := COALESCE(v_venda.participante_secundario_fracao_percentual, (v_venda.snapshot_venda->>'fracao_secundario')::numeric, (v_venda.snapshot_venda#>>'{dados_simulacao,fracao_secundario}')::numeric, 0);
  ...
  v_modo_cronograma_sec := COALESCE((v_venda.snapshot_venda->>'cronograma_secundario'), (v_venda.snapshot_venda#>>'{dados_simulacao,cronograma_secundario}'), 'SEGUIR_PRINCIPAL');
  ...
  ELSIF (v_venda.snapshot_venda#>>'{dados_simulacao,percentual_franqueadora}') IS NOT NULL AND (v_venda.snapshot_venda#>>'{dados_simulacao,percentual_franqueadora}') ~ '^[0-9]+(.[0-9]+)?$' THEN
    v_percentual := (v_venda.snapshot_venda#>>'{dados_simulacao,percentual_franqueadora}')::numeric;
